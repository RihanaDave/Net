...
using Pendar.SIP.Stack;

namespace Pendar.SIP.Debug
{
    /// <summary>
    /// SIP debug UI.
    /// </summary>
    public class wfrm_SIP_Debug : Form
    {
        ...
        public wfrm_SIP_Debug(SIP_Stack stack)
        {
            ...
            m_pStack = stack;
            m_pStack.Logger.WriteLog += new EventHandler<Pendar.Log.WriteLogEventArgs>(Logger_WriteLog);
        ...
        private void Logger_WriteLog(object sender,Pendar.Log.WriteLogEventArgs e)
        {
            if(!this.Visible){
                return;
            }
