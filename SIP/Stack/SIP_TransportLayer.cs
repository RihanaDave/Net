...
using Pendar.DNS;
using Pendar.DNS.Client;
using Pendar.SIP.Message;
using Pendar.UDP;
using Pendar.TCP;
using Pendar.STUN.Client;

namespace Pendar.SIP.Stack
{
    /// <summary>
            ...
            try{
                ...
                // SIP request.
                    ...
                    */
                    ...
                    ////////////////////////////////////////////////////////////////////////////////////////////////////////
                    //Pass ACK to its matched Dialog, not transaction: TEST
                    if (transaction != null && Encoding.UTF8.GetString(message, 0, 3).ToUpper().StartsWith("ACK") )
                    {
                        SIP_Dialog dialog = m_pStack.TransactionLayer.MatchDialog(request);
                        // Pass request to dialog.
                        //m_pStack.Logger.AddText("### Pass ACK request to its dialog.");
                        if (dialog != null)
                        {
                    ...
                    if(transaction != null){
                        ...
                        //m_pStack.Logger.AddText("### Pass request to matched server transaction.");
                        ...
                        SIP_Dialog dialog = m_pStack.TransactionLayer.MatchDialog(request);
                        // Pass request to dialog.
                        //m_pStack.Logger.AddText("### Pass request to dialog.");
                        if(dialog != null){
                            processed = dialog.ProcessRequest(new SIP_RequestReceivedEventArgs(m_pStack,flow,request));
                        }
                    }
